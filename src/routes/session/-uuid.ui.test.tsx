...
describe("Session detail page", () => {
    ...
    mswTest("renders gamemode selector", async () => {
        ...
        await expect
            .element(screen.getByLabelText("Gamemode"))
            .toBeInTheDocument();
    });
    ...
    mswTest("renders time interval picker", async () => {
        ...
        await expect
            .element(screen.getByText("Current interval"))
            .toBeInTheDocument();
    });
    ...
    mswTest("shows related stat columns for FKDR", async () => {
        ...
        await expect
            .element(screen.getByText("Finals").first())
            .toBeInTheDocument();
        await expect
    ...
    mswTest("rate mode shows /hour suffix on column headers", async () => {
        ...
        await expect
            ...
            .toBeInTheDocument();
        await expect
            .element(screen.getByText("Wins/hour").first())
            .toBeInTheDocument();
        // Non-linear stat FKDR should NOT have /hour suffix
        await expect
            .element(screen.getByText("FKDR").first())
            .toBeInTheDocument();
    });
    ...
    mswTest(
        ...
        async ({ worker }: { worker: import("msw/browser").SetupWorker }) => {
            worker.use(
                http.post(
                    "http://localhost:5173/flashlight/v1/sessions",
                    () => {
    ...
    mswTest("stat selector changes URL params", async () => {
        ...
        await screen
            .getByRole("option", { name: "Kills", exact: true })
            .click();
    ...
    mswTest("gamemode selector changes URL params", async () => {
        ...
        await expect
            .poll(() =>
                new URLSearchParams(window.location.search).get("gamemode"),
            )
            .toBe("solo");
    });
    ...
    mswTest(
        "session table mode toggle switches between Total and Rate",
        async () => {
            const { screen } = await renderAppRoute(sessionUrl);
    ...
    mswTest(
        "show in history explorer link includes correct params",
        async () => {
            await renderAppRoute(sessionUrl);
    ...
    mswTest("wins stat shows games and wins columns", async () => {
        ...
        await expect
            .element(screen.getByText("Games").first())
            .toBeInTheDocument();
        await expect
    ...
    mswTest("changing stat to kdr shows kills and deaths columns", async () => {
        ...
        await expect
            .element(screen.getByText("Kills").first())
            .toBeInTheDocument();
        await expect
            .element(screen.getByText("Deaths").first())
            .toBeInTheDocument();
    });
