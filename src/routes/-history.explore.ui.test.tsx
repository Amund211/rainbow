...
describe("History Explore page", () => {
    ...
    mswTest("renders time filter chips", async () => {
        ...
        await expect
            .element(screen.getByText("This month"))
            .toBeInTheDocument();
    });
            ...
            .poll(() => {
                return (
                    ...
                    document.querySelector(
                        "label:has(+ .MuiPickersInputBase-root)",
                    )
                );
            })
            .toBeInTheDocument();
    ...
    mswTest("renders chart title with stat label", async () => {
        ...
        await expect
            .element(
                screen.getByText(
                    `${USERS.player1.username}'s final kill/death ratio`,
                ),
            )
            .toBeInTheDocument();
    });
    ...
    mswTest(
        "renders multiple user chips when multiple UUIDs provided",
        async () => {
        ...
        await expect
            .poll(() => {
                return document.querySelector(
                    'meta[content*="Compare the stats"]',
                );
            })
            .toBeInTheDocument();
    });
    ...
    mswTest(
        "Today chip is highlighted when date range matches today",
        async () => {
            const now = new Date();
    ...
    mswTest(
        "session page button navigates to session page when clicked",
        async () => {
            await renderAppRoute(historyUrl);
    ...
    mswTest("renders all time filter chips", async () => {
        ...
        await expect
            .element(screen.getByText("Last month"))
            .toBeInTheDocument();
