...
describe("Layout - Desktop navigation", () => {
    mswTest("renders all route links in sidebar", async () => {
        ...
        await expect
            .element(screen.getByText("Session stats"))
            .toBeInTheDocument();
        await expect
            .element(screen.getByText("History explorer"))
            .toBeInTheDocument();
        await expect
            .element(
                screen
            ...
            .toBeInTheDocument();
        await expect
            .element(screen.getByText("Downloads").first())
            .toBeInTheDocument();
        await expect
            .element(screen.getByText("Settings").first())
            .toBeInTheDocument();
        await expect
            .element(screen.getByText("About").first())
            .toBeInTheDocument();
    });
...
describe("Layout - Mobile navigation", () => {
    ...
    mswTest("clicking History explorer navigates", async () => {
        ...
        await openBurgerMenu(screen);
        await screen
            .getByRole("menuitem", { name: "History explorer" })
            .click();
...
describe("Layout - active route highlighting updates on navigation", () => {
    mswTest(
        "active link changes when navigating to a different route",
        async () => {
            await page.viewport(1280, 720);
...
describe("Layout - sidebar navigation links include current user", () => {
    mswTest(
        "session stats link includes currentUser UUID when set",
        async () => {
            await page.viewport(1280, 720);
    ...
    mswTest(
        "history explorer link includes currentUser UUID when set",
        async () => {
            await page.viewport(1280, 720);
            localStorage.setItem("currentUser", USERS.player1.uuid);
            await renderAppRoute("/about");
...
describe("Layout - mobile navigation links include current user", () => {
    mswTest(
        "session stats mobile menu link includes currentUser UUID",
        async () => {
            await page.viewport(375, 667);
            localStorage.setItem("currentUser", USERS.player1.uuid);
            const { screen } = await renderAppRoute("/about");
                ...
                .poll(() => {
                    ...
                    return Array.from(menuItems).some(
                        (item) =>
                            ...
                            item
                                .getAttribute("href")
                                ?.includes(USERS.player1.uuid) === true,
                    );
                })
                .toBe(true);
        },
