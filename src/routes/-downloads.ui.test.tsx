...
describe("Downloads page", () => {
    ...
    mswTest("renders version column headers", async () => {
        ...
        await expect
            .element(screen.getByText("Operating System"))
            .toBeInTheDocument();

        await expect
            .element(screen.getByText("Released At"))
            .toBeInTheDocument();
    });
    ...
    mswTest(
        "archive table contains download links pointing to GitHub",
        async () => {
    ...
    mswTest("all three OS types appear in the archive table", async () => {
        ...
        await expect
            .element(screen.getByText("Windows").first())
            .toBeInTheDocument();
        await expect
            .element(screen.getByText("Mac OS").first())
            .toBeInTheDocument();
        await expect
            .element(screen.getByText("Linux").first())
            .toBeInTheDocument();
    });
    ...
    mswTest(
        "latest download link points to correct GitHub release URL",
        async () => {
            await renderAppRoute("/downloads");
            ...
            await expect
                .poll(() => {
                    ...
                    return link
                        ?.getAttribute("href")
                        ?.includes(
                            "github.com/Amund211/prism/releases/download/v1.11.0",
                        );
                })
                .toBe(true);
        },
